...
{
	cout << "Test lexicographic order" << endl << endl << "Print based on line and col" << endl;
	...
	cout << endl << endl << endl;
	assert(mi.valid() == false);
}

void testLineIterator()
{
	cout << "Test line iterator on line index 1" << endl;
	...
	MatrixIterator mi = m.iterator(1);
	...
	assert(mi.valid() == true);
	assert(mi.getCurrent() == 10);
	
	mi.next();
	assert(mi.getCurrent() == 6);
